import adjust_lon from 'proj4/lib/common/adjust_lon';
import HALF_PI from 'proj4/lib/constants/values';
...
export function init() {
    ...
    //New pole
    this.o_lat_p = this.o_lat_p; //Latitude of the North pole of the unrotated source CRS, expressed in the rotated geographic CRS
    this.o_lon_p = this.o_lon_p; //Longitude of the North pole of the unrotated source CRS, expressed in the rotated geographic CRS

    //Rotate about point
    this.o_alpha = this.o_alpha; //Angle to rotate the projection with.
    this.o_lon_c = this.o_lon_c; //Longitude of the point the projection will be rotated about.
    this.o_lat_c = this.o_lat_c; //Latitude of the point the projection will be rotated about.

    //New “equator” points
    this.lon_1 = this.lon_1; //Longitude of first point.
    this.lat_1 = this.lat_1; //Latitude of first point.
    this.lon_2 = this.lon_2; //Longitude of second point.
    this.lat_2 = this.lat_2; //Latitude of second point.
    ...
    innerProjection = "+proj=" + this.o_proj;
    ...
    if(this.o_alpha !== 0) {
    ...
    else if(this.o_lat_p !== 0)  { //New pole specified
        //L216
        ...
        Q.lamp = Math.atan2(Math.cos(phi1) * Math.sin(phi2) * Math.cos(lam1) - Math.sin(phi1) * Math.cos(phi2) * Math.cos(lam2), 
    ...
    switch(Q.projectionType) {
        ...
        case projectionType.transverse: {
            ...
            if(lam < Number.MAX_VALUE) {
                cosphi = Math.cos(phi);
                t = lam - Q.lamp;
                lam = Math.atan2(cosphi * Math.sin(t), -Math.sin(phi));
                phi = Math.asin(cosphi * Math.cos(t));
            }
            p.x = lam;
            p.y = phi;
